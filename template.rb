...
def apply_template!
    ...
    after_bundle do
      ...
      insert_into_file "config/importmap.rb", after: 'pin_all_from "app/javascript/controllers", under: "controllers"' do
        <<-RUBY.strip_heredoc
          \n
          # Scripts relacionados ao template 

          pin 'plugins.bundle', to: 'plugins/global/plugins.bundle.js', preload: true
          pin 'scripts.bundle', to: 'scripts.bundle.js', preload: true
          pin 'fullcalendar.bundle', to: 'plugins/custom/fullcalendar/fullcalendar.bundle.js', preload: true
          pin 'datatables.bundle', to: 'plugins/custom/datatables/datatables.bundle.js', preload: true
          pin 'widgets.bundle', to: 'widgets.bundle.js', preload: true
          pin 'widgets', to: 'custom/widgets.js', preload: true

          # Scripts relacionados ao projeto
          pin 'softmedical', to: 'softmedical.js', preload: true
        RUBY
      end
...
def add_template_repository_to_source_path
    if __FILE__ =~ %r{\Ahttps?://}
      require "tmpdir"
      source_paths.unshift(tempdir = Dir.mktmpdir("start-softmedical-"))
      at_exit { FileUtils.remove_entry(tempdir) }
      git clone: [
        "--quiet",
        "https://isaahmdantas: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@github.com/isaahmdantas/start-softmedical.git",
        tempdir
      ].map(&:shellescape).join(" ")
  
      if (branch = __FILE__[%r{start-softmedical/(.+)/template.rb}, 1])
